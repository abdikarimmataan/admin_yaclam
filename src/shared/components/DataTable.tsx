...
type DataTableProps<T> = {
  title?: string;
  showTotal?: boolean;
  /** `stacked`: actions row, then search. */
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-gray-100 bg-white shadow-sm">
      {toolbarLayout === "stacked" ? (
        <div className="flex flex-col gap-2.5 border-b border-gray-100 p-4">
          {actions}
          <input
            type="text"
            value={search}
        ...
        <div className="flex flex-col gap-2.5 border-b border-gray-100 p-4 sm:flex-row sm:items-center sm:justify-between">
          {(title || showTotal) && (
            <div>
              ...
              {showTotal && <p className="text-sm text-gray-500">Total: {totalRows}</p>}
            </div>
          )}
          ...
          <thead className="border-y border-gray-100 bg-gray-50">
            <tr>
              ...
              {columns.map((col) => (
                <th
                  key={col.key}
                  className={`px-4 py-2.5 text-sm font-medium text-gray-500 ${col.className ?? "text-left"}`}
                >
                  {col.header}
                </th>
              ))}
