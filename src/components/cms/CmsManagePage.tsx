...
import type { CmsModuleConfig, FormField } from "@/config/api-modules";
...
export function CmsManagePage({ config }: CmsManagePageProps) {
  const api = useMemo(() => createCmsApi(config.apiPath), [config.apiPath]);
  ...
  const handleToggleStatus = async (item: CmsRecord) => {
    ...
    setBusyId(String(item.id));
    try {
      await api.updateStatus(String(item.id), !item.status);
      await fetchItems();
    } catch (err) {
