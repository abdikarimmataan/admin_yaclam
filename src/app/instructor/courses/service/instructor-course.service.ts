...
import type { CourseListParams, CourseUploadFiles } from "@/app/courses/service/course.service";

const API_BASE_URL =
  process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:9000/api";

const INSTRUCTOR_COURSE_PATH = `${COURSE_API_PATH}/instructor`;
...
export const instructorCourseApi = {
  getAll(params: CourseListParams = {}) {
    return api
      .get<PaginatedResponse<CourseRecord> | { message?: string }>(
        `${INSTRUCTOR_COURSE_PATH}/getAll${buildQuery(params)}`
      )
  ...
  getById(id: string) {
    return api
      .get<CourseRecord>(`${INSTRUCTOR_COURSE_PATH}/getById/${id}`)
    ...
    if (hasFiles) {
      if (recordId) {
        return multipartSave(
          "PATCH",
          `${INSTRUCTOR_COURSE_PATH}/update/${recordId}`,
          payload,
          files
        );
      }
      return multipartSave("POST", `${INSTRUCTOR_COURSE_PATH}/create`, payload, files);
    }
  ...
  uploadThumbnail(file: File) {
    return uploadFormFile(`${INSTRUCTOR_COURSE_PATH}/upload/thumbnail`, "thumbnail", file);
  },

  uploadVideo(file: File) {
    return uploadFormFile(`${INSTRUCTOR_COURSE_PATH}/upload/video`, "video", file);
  },
  ...
  uploadLessonVideo(
    ...
    return uploadFormFile(
      `${INSTRUCTOR_COURSE_PATH}/${courseId}/curriculum/lesson-video`,
      "video",
      file,
  ...
  updateStatus(id: string, status: boolean) {
    return api.patch<CourseRecord, { status: boolean }>(
      `${INSTRUCTOR_COURSE_PATH}/status/${id}`,
      { status }
    );
  },
  ...
  updateVisible(id: string, isVisible: boolean) {
    return api.patch<CourseRecord, { isVisible: boolean }>(
      `${INSTRUCTOR_COURSE_PATH}/update/${id}`,
      { isVisible }
    );
  },

  remove(id: string) {
    return api.delete<{ message?: string }>(`${INSTRUCTOR_COURSE_PATH}/delete/${id}`);
  },

  saveCurriculum(courseId: string, curriculum: CourseModule[]) {
    return api
      .patch<CourseRecord, { curriculum: CourseModule[] }>(
        `${INSTRUCTOR_COURSE_PATH}/update/${courseId}`,
        { curriculum }
      )
      .then((row) => normalizeCmsRecord(row) as CourseRecord);
  },
  ...
  saveResources(courseId: string, payload: ResourcesSavePayload) {
    ...
    if (files.length > 0) {
      ...
      return fetch(`${API_BASE_URL}${INSTRUCTOR_COURSE_PATH}/update/${courseId}`, {
        method: "PATCH",
        headers: authHeaders(),
        body: fd,
