...
type InstructorCourseResourcesEditorProps = {
  courseId: string;
  courseTitle: string;
  onBack: () => void;
  onSaved: () => void;
...
export function InstructorCourseResourcesEditor({
  courseId,
  courseTitle,
  onBack,
  onSaved,
}: InstructorCourseResourcesEditorProps) {
  ...
  const save = async () => {
    ...
    try {
      const saved = await instructorCourseApi.saveResources(courseId, payload);
