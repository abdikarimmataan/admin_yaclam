...
type InstructorCourseCurriculumPanelProps = {
  courseId: string;
...
export function InstructorCourseCurriculumPanel({
  courseId,
  lessonIdPrefix,
  curriculum,
  errors = {},
  onChange,
}: InstructorCourseCurriculumPanelProps) {
  ...
  const uploadLessonVideo = async (
    ...
    setUploadMessage("");
    try {
      const res = await instructorCourseApi.uploadLessonVideo(
        courseId,
        moduleIndex,
        lessonIndex,
        file
