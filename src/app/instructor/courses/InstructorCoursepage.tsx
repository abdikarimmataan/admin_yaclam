...
import { InstructorCourseCurriculumEditor } from "@/app/instructor/courses/components/InstructorCourseCurriculumEditor";
import { InstructorCourseFormEditor } from "@/app/instructor/courses/components/InstructorCourseFormEditor";
import { InstructorCourseResourcesEditor } from "@/app/instructor/courses/components/InstructorCourseResourcesEditor";
...
export function InstructorCoursepage() {
  ...
  const openShow = async (item: CourseRecord) => {
    ...
    try {
      const record = await instructorCourseApi.getById(String(item.id));
  ...
  const handleToggleVisible = async (item: CourseRecord) => {
    ...
    setBusyId(String(item.id));
    try {
      await instructorCourseApi.updateVisible(String(item.id), nextVisible);
      await fetchData();
    } catch (err) {
  ...
  if (view === "resources" && resourcesCourseId) {
    return (
      <InstructorCourseResourcesEditor
    ...
    <div>
      <div className="mb-4">
        ...
        <p className="mt-1 text-sm text-slate-500">
          Create and manage your courses, curriculum, and resources.
        </p>
      </div>
