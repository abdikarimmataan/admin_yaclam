...
import { getProgramLabel } from "@/app/program/model/program.model";

type ProgramTableProps = {
  loading: boolean;
  paginated: ProgramRecord[];
  startIdx: number;
  search: string;
  page: number;
  pageSize: number;
  ...
  onEdit: (item: ProgramRecord) => void;
  onDelete: (item: ProgramRecord) => void;
  onToggleVisible: (item: ProgramRecord) => void;
};
              ...
              paginated.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 font-medium text-slate-900">{getProgramLabel(item)}</td>
                    <td className="px-3 py-2">
                      <label className="inline-flex cursor-pointer items-center gap-2">
                        <input
                          ...
                          aria-label={`Toggle visibility for ${getProgramLabel(item)}`}
                        />
