...
import { getInstructorRoleLabel } from "@/app/instructor-roles/model/instructor-role.model";

type InstructorRoleTableProps = {
  loading: boolean;
  paginated: InstructorRoleRecord[];
  startIdx: number;
  search: string;
  page: number;
  pageSize: number;
  ...
  onEdit: (item: InstructorRoleRecord) => void;
};

export function InstructorRoleTable({
  loading,
  paginated,
  startIdx,
  search,
        ...
        <table className="w-full min-w-[560px] text-xs">
          <thead>
            <tr className="border-b border-slate-100">
              {["No.", "Name", "Description", "Actions"].map((h) => (
                <th
                  key={h}
