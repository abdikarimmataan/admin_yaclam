...
import {
  ...
  getUniversityLocationLabel,
  type UniversityLocationRecord,
} from "@/app/university-location/model/university-location.model";

type UniversityLocationTableProps = {
  loading: boolean;
  paginated: UniversityLocationRecord[];
  startIdx: number;
  search: string;
  page: number;
  pageSize: number;
  ...
  onEdit: (item: UniversityLocationRecord) => void;
  onDelete: (item: UniversityLocationRecord) => void;
  onToggleVisible: (item: UniversityLocationRecord) => void;
};

export function UniversityLocationTable({
  loading,
  paginated,
  startIdx,
  search,
        ...
        <table className="w-full min-w-[640px] text-xs">
          <thead>
            <tr className="border-b border-slate-100">
              {["No.", "Name", "Country", "Visible", "Actions"].map((h) => (
                <th
                  key={h}
          ...
          <tbody>
            {loading ? (
              ...
              paginated.map((item, i) => {
                ...
                const label = getUniversityLocationLabel(item);
