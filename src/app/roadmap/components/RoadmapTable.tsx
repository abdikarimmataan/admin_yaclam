...
import type { RoadmapRecord } from "@/app/roadmap/model/roadmap.model";
import { getRoadmapLabel } from "@/app/roadmap/model/roadmap.model";

type RoadmapTableProps = {
  loading: boolean;
  paginated: RoadmapRecord[];
  startIdx: number;
  search: string;
  page: number;
  pageSize: number;
  ...
  onEdit: (item: RoadmapRecord) => void;
  onToggleVisible: (item: RoadmapRecord) => void;
};
        ...
        <table className="w-full min-w-[560px] text-xs">
          <thead>
            <tr className="border-b border-slate-100">
              {["No.", "Title", "Demand", "Visible", "Actions"].map((h) => (
                <th
                  key={h}
              ...
              paginated.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 font-medium text-slate-900">{getRoadmapLabel(item)}</td>
                    <td className="px-3 py-2 text-slate-600">{String(item.demand ?? "—")}</td>
                    <td className="px-3 py-2">
                      <label className="inline-flex cursor-pointer items-center gap-2">
                        <input
                          ...
                          aria-label={`Toggle visibility for ${getRoadmapLabel(item)}`}
                        />
