...
type BlogCategoryTableProps = {
  loading: boolean;
  paginated: BlogCategoryRecord[];
  startIdx: number;
  search: string;
  page: number;
  pageSize: number;
  ...
  onEdit: (item: BlogCategoryRecord) => void;
  onToggleVisible: (item: BlogCategoryRecord) => void;
};
        ...
        <table className="w-full min-w-[560px] text-xs">
          <thead>
            <tr className="border-b border-slate-100">
              {["No.", "Name", "Color", "Visible", "Actions"].map((h) => (
                <th
                  key={h}
              ...
              paginated.map((item, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-slate-600">{String(item.color ?? "—")}</td>
                    <td className="px-3 py-2">
                      <label className="inline-flex cursor-pointer items-center gap-2">
                        <input
                          ...
                          aria-label={`Toggle visibility for ${getBlogCategoryLabel(item)}`}
                        />
