...
type InstructorTableProps = {
  ...
  onEdit: (item: InstructorRecord) => void;
  onToggleStatus: (item: InstructorRecord) => void;
};
        ...
        <table className="w-full min-w-[760px] text-xs">
          <thead>
            <tr className="border-b border-slate-100">
              {["No.", "Instructor", "Email", "Role", "Active", "Actions"].map((h) => (
                <th
                  key={h}
          ...
          <tbody>
            {loading ? (
              ...
              paginated.map((item, i) => {
                ...
                const photo = resolveAssetUrl(String(item.photo ?? ""));
                    ...
                    <td className="px-3 py-2">
                      <div className="flex items-center gap-2">
                        {photo ? (
                          ...
                          <span className="grid h-8 w-8 place-items-center rounded-full bg-slate-100 text-[10px] font-bold text-slate-600">
                            {getInstructorLabel(item).slice(0, 2).toUpperCase()}
                          </span>
                        )}
                        <span className="font-medium text-slate-900">{getInstructorLabel(item)}</span>
                      </div>
                    </td>
                    <td className="px-3 py-2 text-slate-600">{String(item.email ?? "—")}</td>
                    <td className="px-3 py-2 text-slate-600">{getInstructorRoleName(item)}</td>
                    <td className="px-3 py-2">
                      <label className="inline-flex cursor-pointer items-center gap-2">
                        <input
                          ...
                          aria-label={`Toggle status for ${getInstructorLabel(item)}`}
                        />
