...
import { getDegreeLevelLabel } from "@/app/degree-level/model/degree-level.model";

type DegreeLevelTableProps = {
  loading: boolean;
  paginated: DegreeLevelRecord[];
  startIdx: number;
  search: string;
  page: number;
  pageSize: number;
  ...
  onEdit: (item: DegreeLevelRecord) => void;
  onDelete: (item: DegreeLevelRecord) => void;
  onToggleVisible: (item: DegreeLevelRecord) => void;
};

export function DegreeLevelTable({
  loading,
  paginated,
  startIdx,
  search,
          ...
          <tbody>
            {loading ? (
              ...
              paginated.map((item, i) => {
                ...
                const label = getDegreeLevelLabel(item);

                return (
                  <tr key={id} className="border-b border-slate-100 last:border-b-0 hover:bg-slate-50/80">
                    ...
                    <td className="px-3 py-2">
                      <label className="inline-flex cursor-pointer items-center gap-2">
                        ...
                        />
                        <span className="text-xs text-slate-600">{isVisible ? "Visible" : "Hidden"}</span>
                      </label>
                    </td>
                    <td className="px-3 py-2">
