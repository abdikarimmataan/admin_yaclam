...
import type { CourseCategoryRecord } from "@/app/course-category/model/course-category.model";
import { getCourseCategoryLabel } from "@/app/course-category/model/course-category.model";

type CourseCategoryTableProps = {
  loading: boolean;
  paginated: CourseCategoryRecord[];
  startIdx: number;
  search: string;
  page: number;
  pageSize: number;
  ...
  onEdit: (item: CourseCategoryRecord) => void;
  onToggleVisible: (item: CourseCategoryRecord) => void;
};
        ...
        <table className="w-full min-w-[560px] text-xs">
          <thead>
            <tr className="border-b border-slate-100">
              {["No.", "Name", "Sort Order", "Visible", "Actions"].map((h) => (
                <th
                  key={h}
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 font-medium text-slate-900">
                      {getCourseCategoryLabel(item)}
                    </td>
                    <td className="px-3 py-2 text-slate-600">{String(item.sortOrder ?? "—")}</td>
                    <td className="px-3 py-2">
                      <label className="inline-flex cursor-pointer items-center gap-2">
                        <input
                          ...
                          aria-label={`Toggle visibility for ${getCourseCategoryLabel(item)}`}
                        />
