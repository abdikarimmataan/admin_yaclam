...
import {
  ...
  locationLabelFromManageRecord,
  offeringsCountFromManageRecord,
  studyAreasSummaryFromManageRecord,
  type ManageUniversityRecord,
} from "@/app/manageuniversity/model/manage-university.model";

type ManageUniversityTableProps = {
  loading: boolean;
  paginated: ManageUniversityRecord[];
  startIdx: number;
  search: string;
  page: number;
  pageSize: number;
  ...
  onEdit: (item: ManageUniversityRecord) => void;
  onDelete: (item: ManageUniversityRecord) => void;
};

export function ManageUniversityTable({
  loading,
  paginated,
  startIdx,
  search,
    ...
    <div className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      <div className="border-b border-slate-100 px-4 py-3">
        <input
          ...
          placeholder="Search manage records..."
        ...
        <table className="w-full min-w-[760px] text-xs">
          <thead>
            <tr className="border-b border-slate-100">
              {["No.", "Name", "Location", "Offerings", "Study Areas", "Actions"].map((h) => (
                <th
                  key={h}
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 text-slate-600">
                      {locationLabelFromManageRecord(item)}
                    </td>
                    <td className="px-3 py-2 tabular-nums text-slate-600">
                      {offeringsCountFromManageRecord(item) || "—"}
                    </td>
                    <td className="px-3 py-2 text-slate-600">
                      {studyAreasSummaryFromManageRecord(item)}
                    </td>
                    <td className="px-3 py-2">
                      <div className="flex justify-end gap-1">
                        ...
                        <button
                          ...
                          aria-label="Delete manage record"
                          title="Delete manage record (keeps university)"
                        >
